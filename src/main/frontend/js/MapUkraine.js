...
export default function MapUkraine(container) {
    ...
    d3.json("json/ukraine.json", function (error, ua) {
        if (error) {
            return console.error(error);
        }

        // draw map
        let countries = topojson.feature(ua, ua.objects.regions);
        ...
        self.projection.fitExtent([[0, 0], [self.width, self.height]], regions);
